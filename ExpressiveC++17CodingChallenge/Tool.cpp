...
// arguments : 1 - the filename of a CSV file 2 - the name of the column to
// overwrite in that file 3 - the string that will be used as a replacement for
// that column 4 - the filename where the output will be written.

// For instance, if the CSV file had a column "City" with various values for the
// entries in the file, calling the tool with the name of the input file, City,
// London and the name of output file would result in a copy of the initial
...
// the console. if the input file does not contain the specified column, the
// program should write "column name doesn't exists in the input file" to the
// console. In both cases, there shouldn't be any output file generated. And if
// the program succeeds but there is already a file having the name specified
// for output, the program should overwrite this file.
...
int main(int argc, char* argv[]) {
  ...
  const auto found_column{std::find(std::begin(column_names),
                                    std::end(column_names), wanted_column)};
  ...
  const auto column_position{
      std::distance(std::begin(column_names), found_column)};

  const std::string wanted_value{
      args[tool::parameter_position::REPLACEMENT_STRING]};
